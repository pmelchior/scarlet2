...
class MultiresRenderer(Renderer):
    def __init__(self, model_frame, obs_frame, padding=None):

        # PREPROCSESS, i.e. Fourier transform
        #####################################
        if padding == None:
            # use 4 times padding
            padding = 4
        ...
        object.__setattr__(self, "model_kpsf", model_kpsf)
        object.__setattr__(self, "obs_kpsf", obs_kpsf)

        # RESAMPLING on target grid
        ###########################
        if padding == None:
            # use 4 times padding
            padding = 4
